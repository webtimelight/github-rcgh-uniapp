			...
			<view class="mix-list">
				<view class="uni-flex-item" v-for="(v,i) in dataList" :key="i">
					<view class="item" @click="goDetail(v)">
						<!-- 上 -->
						<view class="hd">
							<image class="img" :src="goodsSrc(v)"  mode="aspectFill" onerror="javascript:this.src=''"></image>
						</view>
						<!-- 中 -->
						<view class="bd">
							<view class="title uni-ellipsis-2">{{v.goods_name}}</view>
							<view class="ext">
								<view class="f-14 text-gray">数量：<text class="text-red date">{{v.exchange_num}}</text></view>
								<view class="extinfo flex justify-between">
									<view class="f-14 text-gray">兑换时间：<text class="text-red num">{{v.exchange_date}}</text></view>
								</view>
							</view>
						</view>
					</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			goDetail(obj){
				uni.navigateTo({
					url:`/pages/mall/exchangeRecords/detail?id=${obj.id}`
				})
			},
			/*下拉刷新 */
			downCallback(){
			...
			async upCallback(page){
				const {data:res}=await this.$http({
					url:`/points.pointsexchangeorderlist.phtml?viewId=json&operation=self&page=${page.num}&rows=${page.size}`
				})
				if(page.num==1){
					this.dataList=[]
				}
